...
testinfra_hosts = AnsibleRunner(
    os.environ['MOLECULE_INVENTORY_FILE']).get_hosts('dns-server-master')
...
def test_bind9_is_configured_as_master(host):
    file = host.file('/etc/bind/named.conf.local').contains('master')
    assert file
